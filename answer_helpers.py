...
def get_response_flags(allow_quiz_answers):
    ...
    all_response_flags = ["y", "n", "yp", "np", "yi", "ni", "yo", "no", "ye", "ne", "u", "uq", "uqy", "uqn", "wd", "x"]
    ...
    assert len(positive_response_flags) + len(negative_response_flags) + len(undecided_response_flags) == len(
        all_response_flags)
    return positive_response_flags, negative_response_flags, undecided_response_flags


def categorize_answers(answers, positive_response_flags, negative_response_flags, undecided_response_flags):
    ca = []
...
def _load_answers(dataset_name, apis, num_query_templates, positive_response_flags, negative_response_flags, undecided_response_flags):
    ...
    for api_name in apis:
        ...
        canswers = categorize_answers(answers, positive_response_flags, negative_response_flags,
                                      undecided_response_flags)
    ...
    full_responses = np.array(api_responses)
...
def load_compact_answers(dataset_name, apis, num_query_templates, positive_response_flags, negative_response_flags, undecided_response_flags):
    full_responses, queries, variable_names = _load_answers(
        dataset_name, apis, num_query_templates, positive_response_flags, negative_response_flags, undecided_response_flags)
    var_to_idx = {name: i for i, name in enumerate(variable_names)}

    num_variables = len(variable_names)
    adj_mat = np.zeros((len(apis), num_query_templates, num_variables, num_variables))
    # note: assumes same pair ordering for all templates
...
def load_alternative(alternative_dataset_name, adj_mat, variable_names, apis, num_query_templates, positive_response_flags, negative_response_flags, undecided_response_flags):
    adj_mat = np.copy(adj_mat)
    alt_full_responses, alt_queries, alt_variable_names = _load_answers(
        alternative_dataset_name, apis, num_query_templates, positive_response_flags, negative_response_flags, undecided_response_flags)
    var_to_idx = {name: i for i, name in enumerate(variable_names)}
