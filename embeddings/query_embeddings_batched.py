...
list_datasets = [
    #"question_answer",
    "answer_statements_questions" # for exp 3
]
statement_datasets = [
    "altitude_questions",
    "cancer_questions",
    ...
    "recovery_questions",
    #"causal_statements_questions", # deprecated by causal concepts
    #"question_answer" # not for exp 1
]
question_datasets = [
...
datasets = [
    #*list_datasets
    *statement_datasets, # -> for exp 1
    *question_datasets, # -> for exp 2
    *list_datasets # for exp 3
    ...
    #("causal_concepts_questions", None, "causal_concepts_10000") # -> for davinci
    #"causal_concepts_questions" # -> for exp 1 - for ada
]
    ...
    for api_name in active_apis:
        ...
        for dataset_name in datasets:
            ...
            for i in index_list:
                ...
                # collect queries as long as we do not have a full batch, or run out of queries
                if len(query_texts) < batch_size and i != len(index_list)-1:
                    continue

                if len(query_texts) == 0:
                    # the whole dataset is already queried.
                    continue

                embeddings = model.query_embedding_batch(query_texts, log_info=batch_indices)

                if embeddings is None:
                    print(f"[Query {i}] No response.")
                    continue
                answer_count += len(query_texts)

                for query_text, idx, e in zip(query_texts, batch_indices, embeddings):
                    answer_path = answer_dir / f"{idx}_embedding.pkl"
                    info = {
                        "api": api_name,
